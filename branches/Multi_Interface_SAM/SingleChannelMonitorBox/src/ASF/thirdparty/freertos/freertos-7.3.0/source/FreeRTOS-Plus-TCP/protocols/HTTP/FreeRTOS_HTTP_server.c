...
BaseType_t handle_POST_from_setpoint_config(struct xWeb_Post_Container * webPost)
{
	...
	bool * currentSystem = CELSIUS; //this is a dummy variable required by updateTempSystem
	...
	{
		updateTempSystem(FAHRENHEIT, &currentSystem);
	}
	writeNewSetpoint(setpoint_int); //this will always be in Celsius
	return WEB_NO_CONTENT;
}
