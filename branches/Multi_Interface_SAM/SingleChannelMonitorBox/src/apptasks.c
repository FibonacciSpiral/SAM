...
#define	mainTCP_SERVER_STACK_SIZE		( configMINIMAL_STACK_SIZE * 14 )
...
static void prvServerWorkTask( void *pvParameters )
{
	...
	const TickType_t fileSystemInitDelay = pdMS_TO_TICKS( 10L );
	
	NetworkInterface_t * pxSamInterface = &( xInterfaces[ 0 ] );
	
	/* Initialise the interface descriptor for WinPCap. 'xInterfaces[ 0 ]' will
     * now hold all the information for WinPCap driver. */
    pxSamInterface = pxFillInterfaceDescriptor( 0, pxSamInterface );
	...
	FreeRTOS_FillEndPoint( pxSamInterface,
	&( xEndPoints[ 0 ] ),
	ucIPAddress,
	ucNetMask,
	ucGatewayAddress,
	...
	ucMACAddress );
        ...
        */
    #if ( ipconfigUSE_IPv6 != 0 )
        {
            ...
            FreeRTOS_FillEndPoint_IPv6( pxSamInterface,
                                        &( xEndPoints[ 1 ] ),
                                        &( xIPAddress ),
                                        &( xPrefix ),
                                        64uL,            /* Prefix length. */
	...
	xTaskNotifyGive( displayHandlerHandle ); //lets the display handler know that the file system is okay to use
