...
BaseType_t xSetEvent = pdFALSE;
	...
	do
	{
		...
		#if( ipconfigFTP_TX_ZERO_COPY == 0 )
		{
			...
			{
				xRc = FreeRTOS_send( pxClient->xTransferSocket, pcFILE_BUFFER, uxCount - uxBytesSent, 0 );
				uxBytesSent += xRc;
			}
		}
		#else /* ipconfigFTP_TX_ZERO_COPY != 0 */
		{
			...
			while(uxBytesSent < uxCount)
			{
				xRc = FreeRTOS_send( pxClient->xTransferSocket, pcBuffer, uxCount - uxBytesSent, 0 );
				uxBytesSent += xRc;
			}
		}
		#endif /* ipconfigFTP_TX_ZERO_COPY */
